...
plot.mfrd <- function(x, model = c("m_s", "m_h", "m_t"),
  ...
  tol_minus <- 1 - tol
  tol_plus <- 1 + tol
  c1_thresh_minus <- ifelse(c1==0, -tol, c1 * tol_minus)
  c1_thresh_plus <- ifelse(c1==0, tol, c1 * tol_plus)
  c2_thresh_minus <- ifelse(c2==0, -tol, c2 * tol_minus)
  c2_thresh_plus <- ifelse(c2==0, tol, c2 * tol_plus)
  x1 <- c(seq(min(frame$x1), c1_thresh_minus, length.out = ifelse(gran > 4, round(gran * ratio1), 2)),
          seq(c1_thresh_plus, max(frame$x1), length.out = ifelse(gran > 4, round(gran * (1 - ratio1)), 2)))
  x2 <- c(seq(min(frame$x2), c2_thresh_minus, length.out = ifelse(gran > 4, round(gran * ratio2), 2)),
          seq(c2_thresh_plus, max(frame$x2), length.out = ifelse(gran > 4, round(gran * (1 - ratio2)), 2)))
  ...
  if (methodname == 'Param'){
    ...
    #     ind1 <- point.in.polygon(newdata$x1, newdata$x2, l.x1 - tol, l.x2 - tol)
    #     ind2 <- point.in.polygon(newdata$x1, newdata$x2, 
    #                              c(c1, c1, min(c1+bw.x1, max(frame$x1)), min(c1+bw.x1, max(frame$x1))) + tol, 
    #                              c(c2, min(frame$x2), min(frame$x2), c2) - tol)
    #     ind3 <- point.in.polygon(newdata$x1, newdata$x2, 
    #                              c(c1, min(c1+bw.x1, max(frame$x1)), c1) + tol, c(c2, c2, min(c2+bw.x2, max(frame$x2))) + tol)
    #     ind4 <- point.in.polygon(newdata$x1, newdata$x2, 
    #                              c(c1, min(frame$x1), min(frame$x1), c1) - tol, 
    #                              c(min(c2+bw.x2, max(frame$x2)), min(c2+bw.x2, max(frame$x2)), c2, c2) + tol)
    #   }else if ((t.design[1] == 'l' || t.design[1] == 'leq') && 
    #             (t.design[2] == 'l' || t.design[2] == 'leq')){
    #     ind1 <- point.in.polygon(newdata$x1, newdata$x2, g.x1 + tol, g.x2 + tol)
    #     ind2 <- point.in.polygon(newdata$x1, newdata$x2, 
    #                              c(c1, max(frame$x1), max(frame$x1), c1) + tol, 
    #                              c(c2, c2, max(c2-bw.x2, min(frame$x2)), max(c2-bw.x2, min(frame$x2))) - tol)
    #     ind3 <- point.in.polygon(newdata$x1, newdata$x2, 
    #                              c(c1, c1, max(c1-bw.x1, min(frame$x1))) - tol, c(c2, max(c2-bw.x2, min(frame$x2)), c2) - tol)
    #     ind4 <- point.in.polygon(newdata$x1, newdata$x2, 
    #                              c(max(c1-bw.x1, min(frame$x1)), max(c1-bw.x1, min(frame$x1)), c1, c1) - tol, 
    #                              c(c2, max(frame$x2), max(frame$x2), c2) + tol)
    #   }else if ((t.design[1] == 'g' || t.design[1] == 'geq') && 
    #             (t.design[2] == 'l' || t.design[2] == 'leq')){
    #     ind1 <- point.in.polygon(newdata$x1, newdata$x2, l.x1 - tol, g.x2 + tol)
    #     ind2 <- point.in.polygon(newdata$x1, newdata$x2, 
    ...
    #                              c(c2, max(frame$x2), max(frame$x2), c2) + tol)
    #     ind3 <- point.in.polygon(newdata$x1, newdata$x2, 
    #                              c(c1, min(c1+bw.x1, max(frame$x1)), c1) + tol, c(c2, c2, max(c2-bw.x2, min(frame$x2))) - tol)
    #     ind4 <- point.in.polygon(newdata$x1, newdata$x2, 
    #                              c(c1, min(frame$x1), min(frame$x1), c1) - tol, 
    #                              c(max(c2-bw.x2, min(frame$x2)), max(c2-bw.x2, min(frame$x2)), c2, c2) - tol)
    #   }else if ((t.design[1] == 'l' || t.design[1] == 'leq') && 
    #             (t.design[2] == 'g' || t.design[2] == 'geq')){
    #     ind1 <- point.in.polygon(newdata$x1, newdata$x2, g.x1 + tol, l.x2 - tol)
    #     ind2 <- point.in.polygon(newdata$x1, newdata$x2, 
    #                              c(c1, max(frame$x1), max(frame$x1), c1) + tol, 
    #                              c(c2, c2, min(c2+bw.x2, max(frame$x2)), min(c2+bw.x2, max(frame$x2))) + tol)
    #     ind3 <- point.in.polygon(newdata$x1, newdata$x2, 
    #                              c(c1, c1, max(c1-bw.x1, min(frame$x1))) - tol, c(c2, min(c2+bw.x2, max(frame$x2)), c2) + tol)
    #     ind4 <- point.in.polygon(newdata$x1, newdata$x2, 
    #                              c(max(c1-bw.x1, min(frame$x1)), max(c1-bw.x1, min(frame$x1)), c1, c1) - tol, 
    #                              c(c2, min(frame$x2), min(frame$x2), c2) - tol)
    #   }
    ...
    #ind.front <- apply(newdata.bw[,c('x1', 'x2')], 1, function(row_i){return(row_i[1] == c1 - tol || row_i[1] == c1 + tol ||
    #                                                                           row_i[2] == c2 - tol || row_i[2] == c2 + tol)})
    # grid points on the frontier
    #newdata.front <- newdata.bw[ind.front,]
    #newdata.front$yhat <- predict(m, newdata = newdata.front)     
    # grid points not on the frontier
    #newdata.notfront <- newdata.bw[!ind.front,]
...
# weird kink for m_t
  ...
  #   # quadrant boundaries
  #   if (model == 'm_s' || model == 'm_h'){
  #     q.x1 <- c(l.x1 - tol, NA, g.x1 + tol, NA, g.x1 + tol, NA, l.x1 - tol)
  #     q.x2 <- c(g.x2 + tol, NA, g.x2 + tol, NA, l.x2 - tol, NA, l.x2 - tol)
  #   }else if (model == 'm_t'){
  #     if ((t.design[1] == 'g' || t.design[1] == 'geq') &&
  #         (t.design[2] == 'g' || t.design[2] == 'geq')){
  #       q.x1 <- c(l.x1 - tol, NA, c(c1, c1, min(c1+bw.x1, max(frame$x1)), min(c1+bw.x1, max(frame$x1))) + tol, NA,
  #                 c(c1, min(c1+bw.x1, max(frame$x1)), c1) + tol, NA,
  #                 c(c1, min(frame$x1), min(frame$x1), c1) - tol)
  #       q.x2 <- c(l.x2 - tol, NA, c(c2, min(frame$x2), min(frame$x2), c2) - tol, NA,
  #                 c(c2, c2, min(c2+bw.x2, max(frame$x2))) + tol, NA,
  #                 c(min(c2+bw.x2, max(frame$x2)), min(c2+bw.x2, max(frame$x2)), c2, c2) + tol)
  #     }else if ((t.design[1] == 'l' || t.design[1] == 'leq') &&
  #               (t.design[2] == 'l' || t.design[2] == 'leq')){
  #       q.x1 <- c(g.x1 + tol, NA, c(c1, max(frame$x1), max(frame$x1), c1) + tol, NA,
  #                 c(c1, c1, max(c1-bw.x1, min(frame$x1))) - tol, NA,
  #                 c(max(c1-bw.x1, min(frame$x1)), max(c1-bw.x1, min(frame$x1)), c1, c1) - tol)
  #       q.x2 <- c(g.x2 + tol, NA, c(c2, c2, max(c2-bw.x2, min(frame$x2)), max(c2-bw.x2, min(frame$x2))) - tol, NA,
  #                 c(c2, max(c2-bw.x2, min(frame$x2)), c2) - tol, NA,
  #                 c(c2, max(frame$x2), max(frame$x2), c2) + tol)
  #     }else if ((t.design[1] == 'g' || t.design[1] == 'geq') &&
  #               (t.design[2] == 'l' || t.design[2] == 'leq')){
  #       q.x1 <- c(l.x1 - tol, NA, c(c1, c1, min(c1+bw.x1, max(frame$x1)), min(c1+bw.x1, max(frame$x1))) + tol, NA,
  #                 c(c1, min(c1+bw.x1, max(frame$x1)), c1) + tol, NA,
  #                 c(c1, min(frame$x1), min(frame$x1), c1) - tol)
  #       q.x2 <- c(g.x2 + tol, NA, c(c2, max(frame$x2), max(frame$x2), c2) + tol, NA,
  #                 c(c2, c2, max(c2-bw.x2, min(frame$x2))) - tol, NA,
  #                 c(max(c2-bw.x2, min(frame$x2)), max(c2-bw.x2, min(frame$x2)), c2, c2) - tol)
  #     }else if ((t.design[1] == 'l' || t.design[1] == 'leq') &&
  #               (t.design[2] == 'g' || t.design[2] == 'geq')){
  #       q.x1 <- c(g.x1 + tol, NA, c(c1, max(frame$x1), max(frame$x1), c1) + tol, NA,
  #                 c(c1, c1, max(c1-bw.x1, min(frame$x1))) - tol, NA,
  #                 c(max(c1-bw.x1, min(frame$x1)), max(c1-bw.x1, min(frame$x1)), c1, c1) - tol)
  #       q.x2 <- c(l.x2 - tol, NA, c(c2, c2, min(c2+bw.x2, max(frame$x2)), min(c2+bw.x2, max(frame$x2))) + tol, NA,
  #                 c(c2, min(c2+bw.x2, max(frame$x2)), c2) + tol, NA,
  #                 c(c2, min(frame$x2), min(frame$x2), c2) - tol)
  #     }
  #   }
  #   q.data <- data.frame(x1 = q.x1, x2 = q.x2)
  #   q.x1_tr <- treat_assign(q.data$x1, c1, t.design[1])
  #   q.x2_tr <- treat_assign(q.data$x2, c2, t.design[2])
  #   q.data$tr1 <- ifelse(is.na(q.data$x1) | is.na(q.data$x2), NA, ifelse(q.x1_tr & !q.x2_tr, 1, 0))
  #   q.data$tr2 <- ifelse(is.na(q.data$x1) | is.na(q.data$x2), NA, ifelse(!q.x1_tr & q.x2_tr, 1, 0))
  #   q.data$trb <- ifelse(is.na(q.data$x1) | is.na(q.data$x2), NA, ifelse(q.x1_tr & q.x2_tr, 1, 0))
  #   q.data$tr <- ifelse(q.data$tr1 == 1 | q.data$tr2 == 1 | q.data$trb == 1, 1, 0)
  #   q.data$quadrant <- interaction(q.x1_tr, q.x2_tr)
  #   q.data$id  <- 1:nrow(q.data)
  #   q.data <- merge(q.data,
  #                    data.frame(
  #                      quadrant = c("0.0", "0.1", "1.0", "1.1"),
  #                      color = c(NA, "#428bca", "#428bca", "#428bca")
  #                    ), all.x = TRUE)
  #   q.data <- q.data[order(q.data$id), ]
  # 
  #   q.data$yhat <- predict(m, q.data[,c('x1','x2','tr1','tr2','trb','tr')])
  #   
  #   q.data$yhat <- apply(q.data[,c('x1','x2','tr1','tr2','trb','tr')], 1, FUN)  
  #   
  #   poly_3d <- as.data.frame(trans3D(q.data$x1, q.data$x2, q.data$yhat, pmat = ele_3d))
  #   polygon(poly_3d$x, poly_3d$y, col = adjustcolor(q.data$color, alpha.f = .2), border = NA)
  # }
  # 
  ...
  #       }else{
  #         poly_3d <- as.data.frame(trans3D(frame$x1, frame$x2, frame$yhat, pmat = ele_3d))
  #         poly_3d <- poly_3d[!is.na(poly_3d$x),]
  #         poly_3d <- poly_3d[chull(x = poly_3d$x, y = poly_3d$y),]
  #       }
  #       polygon(poly_3d$x, poly_3d$y, col = adjustcolor(frame$color, alpha.f = .2), border = NA)
  #     }
  #   )
  ...
  # weird kinks in mt plot
  # smaller dots for data
