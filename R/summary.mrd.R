...
summary.mrd <- function(object, digits = max(3, getOption("digits") - 3), ...) {
  ...
  if ('center' %in% names(object)){
    ...
    center_summary = summary(center_obj)
  ...
  if ('univ' %in% names(object)){
    ...
    univR_summary = summary(univR_obj)
    ...
    univM_summary = summary(univM_obj)
  ...
  if ('front' %in% names(object)){
    ...
    front_summary = summary(front_obj)
