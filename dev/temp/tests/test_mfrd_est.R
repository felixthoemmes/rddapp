...
test_that("constant treatment effects (model 1)", {
  stata_dat <- read.csv('~/rdd/reference/mrdd/stata/Curtis_seed100_m1_dat.csv')
  stata_dat_h <- read.csv('~/rdd/reference/mrdd/stata/Curtis_seed100_m1_dat_h.csv')

  dat <- foreign::read.dta('~/rdd/reference/mrdd/stata/Curtis_seed100_m1.dta')
  dat <- dat[c('readav','mathav','m_posty')]
  names(dat) = c('x1','x2','y')

  mod <- mfrd_est(y = dat$y, x1 = dat$x1, x2 = dat$x2, c1 = 40, c2 = 60, frame = T)  
  ...
  expect_equal(stata_est, mod[names(stata_est)], tolerance = 2e-4)
})

test_that("constant treatment effects (model 2)", {
  stata_dat <- read.csv('~/rdd/reference/mrdd/stata/Curtis_seed100_m2_dat.csv')
  stata_dat_h <- read.csv('~/rdd/reference/mrdd/stata/Curtis_seed100_m2_dat_h.csv')

  dat <- foreign::read.dta('~/rdd/reference/mrdd/stata/Curtis_seed100_m2.dta')
  dat <- dat[c('readav','mathav','m_posty')]
  names(dat) = c('x1','x2','y')
  ...
  expect_equal(stata_est, mod[names(stata_est)], tolerance = 6e-5)
})

test_that("constant treatment effects (model 3)", {
  stata_dat <- read.csv('~/rdd/reference/mrdd/stata/Curtis_seed100_m3_dat.csv')
  stata_dat_h <- read.csv('~/rdd/reference/mrdd/stata/Curtis_seed100_m3_dat_h.csv')

  dat <- foreign::read.dta('~/rdd/reference/mrdd/stata/Curtis_seed100_m3.dta')
  dat <- dat[c('readav','mathav','m_posty')]
  names(dat) = c('x1','x2','y')
