...
## Online use
The Shiny environment of the "rddapp" can be accessed online, here: https://rddapp.shinyapps.io/shinyrdd/. Untested features will first be introduced to the beta version, which can be accessed online, here: https://rddapp.shinyapps.io/shinyrdd_beta/.

The online Shiny enviorment is completely point-and-click, and no programming in R is required. Data for analysis can be uploaded directly to the Shiny server. However, sensitive data should not be transferred over the Internet, and the use of an offline client is preferred.

## Offline use
The easiest way to use the Shiny application offline, is to download the offical R package from CRAN (https://cran.r-project.org/web/packages/rddapp/index.html), and execute the function shiny_run(). This will load the Shiny enviroment in the local browser, without the need of being online, or transfering any data over the Internet.

## Interface
At the very top of the Shiny app are three tabs, labeled *"Model"*, *"Power"*, and *"More"*. The Model tab contains all routines for estimation, the Power tab contains all routines for power analysis, and the More tab, contains this manual, and a simple "About" page.  

## Model
The model tab itself is subdivided into a data panel on the left, and a dynamic results panel on the right. The dynamic panel is not shown until data has been uploaded.

### Data
Typically, the user will begin by uploading the data using the pull-down menu in the data upload tab. The choices are to upload either a .SAV file (native file format of SPSS), or a CSV file (comma separated values). For each of these two choices, the user can further fine-tune the data upload, by clicking on the small button (in the shape of a gear) at the top right. The gear button reveals additional options, and is updated dynamically based on user-chosen options. The "Browse" button appears automatically when the user chooses a .SAV or .CSV file. The Browse button opens a file explorer from which the user can navigate to the datafile for upload.
...
Finally, the user can indicate that the uploaded file has been multiply imputed (typically to deal with missing values). The user needs to provide the Imputation ID variable, and all analyses will then be performed on the multiply imputed datafile. In particular this means, that all analyses will be performed on each imputation, then estimates from the imputation will be averaged, and standard errors will be derived using Rubin's rules (1987), taking into account both the variation within imputations, and between imputations. *Is more detailed needed here? And if so, should it appear here, or in some sort of technical appendix??*
...
#### Treatment Design
ShinyRDD allows the user fine-grained control over the actual treatment assignment mechanism. The treatment variable is the variable that encodes which treatment was _actually_ received by the participants. In some designs the assignment rule, and the actually received treatment may not be identical. This variable is entered in the box labeled "Treatment receipt".

The treatment mechanism is described by (a series) of "if" statements. In the pull-down labelled "primary assignment" the user inputs the variable on which the assignment is based. **In the pull-down below, the user can choose under which condition the treatment was applied, and supply the value of the actual cut-off that was used to determine the assignment to either treatment or control condition. The user can further change the "smaller equals" and "larger equals" sign, by clicking on it. As an example, choosing an assignment variable and then selecting, "larger equals" with cut-off value 40, would mean that treatment was assigned to those individuals that were equal or larger to 40. Clicking the gear button, reveals one additional option. The user can specify whether individuals that were right at the cut-off were assigned to the treatment or control. By default, this option is clicked, but when unclicked, the "larger equal" sign (or "smaller equal" sign) turns to a strictly "larger" (or "smaller") sign.**
