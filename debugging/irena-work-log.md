...
## 2019-12-23 (1 hr)
* decreased resolution of exported pngs to get text annotations to a more resonable size
  [] put box with background around some legends for legibility
* started looking at formatting issues with exported CSVs
* table 1.2:
  * it looks like an encoding error
  * table reads fine when opened with UTF-8 encoding but excel opens in ANSI by default?
  * adding bom = 'true' as one of csv download button options worked! (it ensures that excel knows what the text encoding of the file is when opening)
...
## 2019-12-31 (2 hrs)
* fixed issue of legends running into plot elements in fig 3.1 [generated by plot.rd()]
  * played around for a while with putting legends in margins, but its not consistent enough (depends on how the user sets up margins)
  * ended up keeping legends within plot area but extending the ylims by adding buffer space to ensure legends usually don't run into plot elements
* used the same trick for legend issue in fig 4.1 in sens_plot(), but it doesn't always work well because of the plotting area getting set up first and then different lines for different models get added... don't have time to do something more sophisticated (somehow access the min and max yvalue of *all* of the models plotted and then add buffer space to those)
  * thought about adding y-lim user input boxes like for fig 4.2, but fig 4.1 isn't implemented the same way, so it's not straightforward to port over what i did for fig 4.2 to fig 4.1
