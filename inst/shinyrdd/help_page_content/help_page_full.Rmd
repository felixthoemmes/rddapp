...
## Interface
At the very top of the Shiny app are three tabs, labeled *"Model"*, *"Power"*, and *"More"*:

* the Model tab contains all estimation modules;
* the Power tab contains all power analysis modules;
* the More tab contains a Help page (with this manual), as well as a simple "About" page with software author information and references.  
...
### Data
Typically, the user will begin by uploading the data using the pull-down menu in the data upload panel (labeled "Data"). First, the user must specify the data file type. The choices are:
...
If the user selects an .SAV file for upload, these options include whether value labels in SPSS should be read into the uploaded datafile: e.g., a user may have coded a categorical variable 0 and 1, and included value labels "male" and "female". If "Use Value Labels" is activated, the actual labels will be displayed, otherwise numbers will be used. The option "Use User's Missing Value" instructs the program to code any user-specified missing values in SPSS to be recoded as NA in the datafile.
