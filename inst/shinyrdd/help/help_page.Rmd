...
The second parameter is the band width, also known as the smoothing parameter. The smoothing parameter simply determines how much data is considered at once, when computing the smooth approximation. A large value results in a lot of data being considered, and the smoother being very flat. A small number results in narrow bands of data being considered when computing local means, and results in a very jagged line. The choice of bandwidth will naturally result in different test statistics, and possibly different inferential decisions. Generally speaking, a bandwidth parameter should be chosen, so that no over- or under-smoothing occurs. To aid in the selection of the parameter, a graphical display is provided, and updated every time the smoothing parameter changes.
