...
The **parametric** model results are subdivided into three rows, where each row gives results of the outcome variable modeled as a function of the assignment variable, but with different functional forms in each case. The available models are:
...
* **Bandwidth**: empty for all parametric models because it only applies to non-parametric models;
...
Users may wish to perform a formal (frequentist) power analysis, to determine a-priori sample sizes, or to construct power curves for various effect sizes, or sample sizes. 
