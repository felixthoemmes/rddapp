...
** Complete model**
qui 
reg  m_posty `av' zreadmath `alltreat' `complete' 
...
** CREATES H VARIABLE *********************************
cap:drop hr* hm* treatment comp tmath tread
...
cap:drop gx1
gen gx1 = streat*treatment + streatm*tmath + strmrav*tmath*(hr - zreadc)

* Function for treatment effect along READING frontier
qui 
gen gx2 = streat*treatment + streatr*tread + strrmav*tread*(hm - zmathc)
...
qui 
cap:drop d x
kdensity zreadav if mathres == 1, gen(x d)  kernel(tri)
cap:drop fx1
kdensity zreadav if mathres == 1, gen(fx1) at(hr) tri nogr
...
qui 
kdensity zmathav if readres == 1, gen(fx2) at(hm) tri nogr
...
* Numerator: Integral for numerator of MATH frontier (x1)
qui 
integ htx1prod hr if hr>=zreadc 
...
* Product for numerator: g(x)*f(x)
qui
 gen htx2prod = htgx2*fx2 if hm>=zmathc

* Numerator: Integral for numerator of READING frontier (x2)
qui 
integ htx2prod hm if hm >=zmathc
